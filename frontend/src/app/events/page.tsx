...
import {
  Calendar, MapPin, Users, Plus, Trash2, Lock, Sparkles,
  X, GraduationCap, Briefcase, Globe, Info, Building2, ExternalLink
} from 'lucide-react';
...
const ANIM_STYLES = `
  ...
  .ev-modal-card { animation: ev-popIn 0.22s ease both; }
  .ev-confirm-modal { animation: ev-confirmPop 0.22s cubic-bezier(0.34,1.56,0.64,1) both; }
  .ev-badge { cursor: pointer; transition: background 0.15s ease, transform 0.15s ease; }
  .ev-badge:hover { transform: scale(1.06); }
`;
...
export default function Events() {
  ...
  // ── Handles internal registration (called from ConfirmModal) ──────────────
  const handleRegister = async (eventId: string) => {
    try {
      await axios.post(`/api/events/${eventId}/register`);
      toast.success('Registered successfully! 🎟️');
  ...
  function ConfirmModal() {
    ...
    return (
      <div className="ev-modal-backdrop" onMouseDown={() => setConfirmModal(null)}>
        <div
          className="ev-confirm-modal bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6"
          onMouseDown={(e) => e.stopPropagation()}
        >
          {/* Ticket icon */}
          <div className="w-16 h-16 ev-grad-btn rounded-2xl flex items-center justify-center mx-auto mb-5 shadow-lg">
          ...
          <p className="text-sm font-bold text-slate-800 text-center mb-5 leading-snug px-2">
            "{confirmModal.eventTitle}"
          </p>

          {/* External link notice */}
          {confirmModal.isExternal ? (
            <div className="mb-5 p-3.5 bg-blue-50 rounded-xl border border-blue-100 flex items-start gap-2.5">
              <ExternalLink className="w-4 h-4 text-blue-500 mt-0.5 shrink-0" />
              <p className="text-xs text-blue-700 font-medium leading-relaxed">
                This event uses an <strong>external registration form</strong>. Clicking confirm will open it in a new tab.
              </p>
            </div>
          ) : (
            <div className="mb-5 p-3.5 bg-emerald-50 rounded-xl border border-emerald-100 flex items-start gap-2.5">
              <span className="text-emerald-500 text-sm mt-0.5 shrink-0">✅</span>
              <p className="text-xs text-emerald-700 font-medium leading-relaxed">
                Your seat will be <strong>confirmed instantly</strong>.
              </p>
            </div>
          )}
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 py-3 border-2 border-slate-200 rounded-xl text-slate-500 hover:text-slate-700 hover:bg-slate-50 font-bold text-sm transition-all"
            >
              Cancel
            </button>
            <button
              ...
              className="flex-1 py-3 ev-grad-btn text-white rounded-xl font-bold text-sm shadow-md active:scale-95 transition-all"
            >
              {confirmModal.isExternal ? 'Open Form 🔗' : 'Yes, Register! 🚀'}
            </button>
          </div>
        </div>
      </div>
...
// Time ko midnight par set kar do taaki sirf din compare ho
currentDate.setHours(0, 0, 0, 0); 

// 1. Naye Events (Jo aaj hain ya aage aayenge)
const upcomingEvents = events.filter((event) => new Date(event.date) >= currentDate);

// 2. Purane Events (Jo beet chuke hain)
    ...
    <>
      ...
      <div className="max-w-6xl mx-auto py-6 sm:py-10 px-4 sm:px-6">
        ...
        {events.length === 0 ? (
          ...
          <>
            {/* =========================================================
                SECTION 1: UPCOMING EVENTS (Active, Clickable)
            ============================================================= */}
            {upcomingEvents.length > 0 && (
              <div className="mb-12">
                <h2 className="text-2xl font-bold text-slate-800 mb-6 flex items-center gap-2 ev-fade-1">
                  <Sparkles className="w-6 h-6 text-blue-500" /> Upcoming Events
                </h2>
                    ...
                    return (
                      <div key={event.id} className={`${fadeClass} ev-card bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col`}>
                        ...
                        <div className="ev-img-wrap h-44 sm:h-48 relative bg-slate-100 overflow-hidden">
                          ...
                          {hasDepts && (
                            <button type="button" onClick={() => setAudienceModal(event)}
                              className="ev-badge absolute top-3 right-3 bg-amber-100/90 backdrop-blur px-2.5 py-1 rounded-full text-[10px] font-bold text-amber-700 shadow-sm flex items-center gap-1">
                              <Building2 className="w-3 h-3" />
                              {batches.departments.length === 1 ? batches.departments[0] : `${batches.departments.length} depts`}
                            </button>
                          )}
                        </div>
                        ...
                        <div className="p-5 sm:p-6 flex-1 flex flex-col">
                          ...
                          <div className="mt-auto pt-4 border-t border-slate-100 flex gap-2">
                            <button
                              onClick={() => setConfirmModal({ eventId: event.id, eventTitle: event.title, isExternal, externalLink: event.registrationLink || undefined })}
                              className="ev-register-btn flex-1 ev-grad-btn text-white py-2.5 sm:py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-1.5"
                            >
                              {isExternal ? <><ExternalLink className="w-3.5 h-3.5" /> Register Now</> : <>Register Now ✨</>}
                            </button>
                            {user?.role === 'ADMIN' && (
                              <button onClick={() => handleDelete(event.id)} className="w-10 h-10 sm:w-auto sm:px-4 sm:py-3 flex items-center justify-center bg-red-50 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all font-medium shrink-0">
                                <Trash2 className="w-4 h-4" /><span className="hidden sm:inline ml-1.5 text-sm font-semibold">Delete</span>
                              </button>
                            )}
                          </div>
            ...
            {/* =========================================================
                SECTION 2: PAST EVENTS (Dull, Disabled Button)
            ============================================================= */}
            {pastEvents.length > 0 && (
              <div className="opacity-80 grayscale-[15%]">
                <h2 className="text-2xl font-bold text-slate-500 mb-6 flex items-center gap-2 ev-fade-1">
                  <Calendar className="w-6 h-6 text-slate-400" /> Past Events
                </h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 lg:gap-8">
                  {pastEvents.map((event, idx) => {
                    ...
                    return (
                      <div key={event.id} className={`${fadeClass} bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col`}>
                        {/* Image */}
                        <div className="h-44 sm:h-48 relative bg-slate-100 overflow-hidden">
                          ...
                          <div className="absolute inset-0 bg-black/10"></div> {/* Dark overlay for past event image */}
                          
                          <button type="button" onClick={() => setAudienceModal(event)} className={`absolute top-3 left-3 ${cfg.badgeBg} backdrop-blur px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${cfg.badgeText} shadow-sm flex items-center gap-1`}>
                            <Info className="w-3 h-3" />
                            {event.targetAudience === 'ALL' ? 'For: Everyone' :
                              event.targetAudience === 'ALUMNI' ? 'For: Alumni' :
                                event.targetAudience === 'STUDENT' ? 'For: Students' : 'For: Custom'}
                        ...
                        {/* Card body */}
                        <div className="p-5 sm:p-6 flex-1 flex flex-col">
                          <h3 className="text-lg sm:text-xl font-bold text-slate-700 mb-1.5 leading-tight">{event.title}</h3>
                          <p className="text-slate-400 text-sm mb-5 line-clamp-2 leading-relaxed">{event.description}</p>
                          ...
                          {/* Actions (DISABLED) */}
                          <div className="mt-auto pt-4 border-t border-slate-100 flex gap-2">
                            <button disabled className="flex-1 bg-slate-100 text-slate-400 py-2.5 sm:py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-1.5 cursor-not-allowed border border-slate-200">
                              Event Completed
                            </button>
                            {user?.role === 'ADMIN' && (
                              <button onClick={() => handleDelete(event.id)} className="w-10 h-10 sm:w-auto sm:px-4 sm:py-3 flex items-center justify-center bg-red-50 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all font-medium shrink-0">
...
//                     {/* External form badge */}
//                     {isExternal && (
//                       <div className="absolute bottom-3 right-3 bg-white/90 backdrop-blur px-2.5 py-1 rounded-full text-[10px] font-bold text-blue-600 shadow-sm flex items-center gap-1">
